...
public enum RefactoringType {
    ...
    REPLACE_EXPECTED_WITH_ASSERT_THROWS("Replace @test(expected) with assertThrows", "Replace @Test(expected) with assertThrows (.+) with (.+) in method (.+) from class (.+)"),

    REPLACE_TRY_FAIL_WITH_RULE("Replace try/catch With Rule", "Replace try/catch With Rule (.+) with (.+) in method (.+) from class (.+)");
